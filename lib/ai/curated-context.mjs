...
//
// movie_news is excluded even though it's a NEWS_CATEGORIES entry. It was
// originally TMDB-sourced, and TMDB's API Terms of Use flatly bar using
// TMDB content in an AI/chatbot application — not something a commercial
// license was stated to cure. movie_news is now sourced from Trakt instead
// (see lib/cron/ingest/movies.mjs — TMDB wanted $149/mo for a commercial
// key over one minor content category), whose staff confirmed on their
// developer forum that commercial use is unrestricted, but their full
// binding Terms of Use couldn't be independently verified from here (the
// page blocks automated fetches) — specifically, whether they have any
// AI/ML restriction of their own was never confirmed either way. Keeping
// the exclusion as the cautious default until someone actually reads
// app.trakt.tv/terms and confirms otherwise; it's cheap to remove later,
// expensive to have gotten wrong. movie_news is still shown to users
// directly on /updates and Discover (app/actions/curatedContent.ts) — this
// exclusion is scoped to the AI assistant's context only.
const AI_FORBIDDEN_CATEGORIES = new Set(["movie_news"]);
export const ALL_CURATED_CATEGORIES = [
  ...FOOTBALL_CATEGORIES,
  ...NEWS_CATEGORIES,
  "match_analysis",
  "match_summary",
].filter(category => !AI_FORBIDDEN_CATEGORIES.has(category));
...
function formatRow(row) {
  ...
  if (row.category === "match_analysis") {
    return `[AI-written analysis, not a guarantee] ${row.title} — ${row.body ?? ""}`;
  }

  if (row.category === "match_summary") {
    return `[AI-written match recap] ${row.title} — ${row.body ?? ""}`;
  }
