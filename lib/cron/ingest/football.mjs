...
// Free tier covers all six of these (confirmed live against
// api.football-data.org — CL added after a visitor asked why a Champions
// League-adjacent result wasn't showing; cup competitions like the FA Cup
// or Community Shield are NOT on the free tier, confirmed against
// football-data.org's coverage page, so those stay out of scope) under the
// same 10 req/min cap this module stays well under — one request per
// competition, per run. Keep this in sync with FOOTBALL_LEAGUES in
// lib/curated-content-categories.mjs, which everything else (interest
// picker, league filters, the "leagues we cover" note) reads from.
const COMPETITIONS = [
