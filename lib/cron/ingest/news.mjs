...
const NEWS_TOPICS = [
  // "football" alone is ambiguous across NewsData.io's US-heavy source mix —
  // confirmed live, it pulled American college football recruiting stories
  // right alongside actual soccer. "soccer" plus specific competition/body
  // names stays unambiguous without needing "football" in the query at all.
  {
    category: "football_news",
    params: { category: "sports", q: "soccer OR \"premier league\" OR uefa OR fifa OR \"la liga\" OR \"champions league\"" },
  },
  // Bare category params (no q) trust NewsData.io's own classifier — which
  // turned out loose enough to file general/disaster stories under
  // "technology" (confirmed live: an Indiana flooding-deaths story came
  // back tagged tech_news). A keyword filter, same fix as football_news
  // above, keeps each category actually on-topic.
  {
    category: "tech_news",
    params: { category: "technology", q: "technology OR tech OR software OR \"artificial intelligence\" OR startup OR gadget OR smartphone" },
  },
  {
    category: "education_news",
    params: { category: "education", q: "education OR school OR university OR student OR classroom OR curriculum" },
  },
