...
export const NEWS_CATEGORY_FILTERS = [
  { value: "all", label: "All" },
  { value: "football_news", label: "Football news" },
...
// lib/cron/ingest/news.mjs) — crypto_news (CryptoPanic), crypto_price,
// betting_prediction, and video_trending don't carry that field, so
// country-matching against them would just find nothing.
...
// since the Aside/Updates interest filters need the same list. Also doubles
// as the "leagues we cover" list shown to visitors (see
// components/feed/LeagueCoverageNote.tsx) — every competition ingested by
// lib/cron/ingest/football.mjs belongs here, and nothing else does.
