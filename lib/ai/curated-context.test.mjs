...
test("formatCuratedContext caps items per category instead of letting one crowd out the rest", () => {
  ...
  rows.push({
    category: "gossip_news",
    title: "A gossip story",
    body: null,
    published_at: "2026-08-17T10:00:00Z",
    metadata: {},
  });
  ...
  const lines = text.split("\n");
  assert.equal(lines.length, 5); // 4 tech_news (capped) + 1 gossip_news
  assert.ok(lines.some(line => line.includes("A gossip story")));
});
