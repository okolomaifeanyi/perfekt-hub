...
import { listMatchesPage } from "@/app/actions/discover";
...
export function MatchListClient() {
  ...
  const { items, loading, loadingMore, hasMore, sentinelRef } = useInfiniteList<UserProps>({
    ...
    fetchPage: async ({ offset, sortMode: mode, limit }) => {
      ...
      return listMatchesPage({ currentUid, offset, sortMode: mode as ListSortMode, limit });
    },
  });

  return (
    <div className="space-y-4">
      <SortToggle value={sortMode} onChange={setSortMode} engagementLabel="Best match" />
      ...
      {loading ? (
        ...
        <Card className="py-8">
          <CardContent className="text-center text-sm text-muted-foreground">
            No match candidates yet — check back once more people join.
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2">
