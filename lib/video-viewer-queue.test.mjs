...
test("buildVideoViewerQueue excludes the viewer's own videos from candidates but keeps currentPost", () => {
  const currentPost = {
    id: "current",
    userId: "viewer-1",
    media: [{ type: "video", src: "current.mp4" }],
  ...
  const feedPosts = [
    currentPost,
    { id: "own-other", userId: "viewer-1", media: [{ type: "video", src: "own.mp4" }], content: "mine" },
    { id: "beta", userId: "someone-else", media: [{ type: "video", src: "beta.mp4" }], content: "beta" },
  ];
  ...
  assert.equal(queue[0].id, "current");
  assert.ok(!queue.some(post => post.id === "own-other"));
  assert.ok(queue.some(post => post.id === "beta"));
});
