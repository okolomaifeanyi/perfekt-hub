...
// generic "what's going on" phrasing. False positives just mean an
// unrelated message pays for a Supabase query it didn't need; false
// negatives mean Nwanne wrongly claims no access again, which is the worse
// failure mode.
const RELEVANCE_KEYWORDS =
  /\b(game|match(?:es)?|score|fixture|kick[\s-]?off|play(?:ing|s)?|result|win(?:s)?|lose|lost|beat|predict(?:ion)?s?|odds|versus|vs\.?|league|table|standings?|news|update[sd]?|happening|trending|latest|headlines?|crypto|bitcoin|ethereum|coin|price|movie(?:s)?|film(?:s)?|cinema|album|song|music|artist|celebrity|gossip|scam|fraud|startup|tech(?:nology)?|gadget|\bai\b|education|school|university|student)\b/i;
